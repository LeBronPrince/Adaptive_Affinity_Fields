...
import os
import sys
sys.path.append("/Semantic Segmentation Code/Adaptive_Affinity_Fields")
...
def main():
    ...
    output_D_G_flat = tf.nn.softmax(output_D_G_flat)
    output_D_T_flat = tf.nn.softmax(output_D_T_flat)


    DLoss1 = utils.general.DLoss(output_D_T, output_D_G)
    GLoss1 = utils.general.GLoss(label_int, output[0], output_D_G,arg.num_classes,arg.kd)
    #DLoss_G = tf.reduce_mean(output_D_G_flat*loss_soft1)
