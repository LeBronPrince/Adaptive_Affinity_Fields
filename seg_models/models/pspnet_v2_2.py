...
def pspnet_v2(x,name,num_classes,is_training,use_global_status,reuse=False):
    ...
    # Build pooling layer results in 1x1 output.
        pool1 = nonlocal_dot(res4, 512, embed=True, softmax=True, scope="nonlocal/scale_1",scale=1)
        pool2 = nonlocal_dot(res4, 512, embed=True, softmax=True, scope="nonlocal/scale_2",scale=2)
        pool3 = nonlocal_dot(res4, 512, embed=True, softmax=True, scope="nonlocal/scale_3",scale=3)
        pool6 = nonlocal_dot(res4, 512, embed=True, softmax=True, scope="nonlocal/scale_6",scale=6)
