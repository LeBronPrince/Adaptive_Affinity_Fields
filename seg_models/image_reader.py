...
def divide_into_parts(image,label, region_size, overlap):
    ...
    combined = tf.concat(axis=2, values=[image, label])
    height = int(region_size[0])
    width = int(region_size[1])
    image_rgb = tf.expand_dims(combined, 0)

    images = tf.concat([image_rgb[:, :height+overlap, :width+overlap, :],
                    image_rgb[:, :height+overlap, width-overlap:, :],
                    image_rgb[:, height-overlap:, :width+overlap, :],
                    image_rgb[:, height-overlap:, width-overlap:, :]],0)
...
def read_labeled_image_list(data_dir, data_list):
  ...
  for line in f:
    try:
      ...
      image = mask = line.strip("\n")
    images.append(data_dir+'leftImg8bit_trainvaltest/' + image)
    masks.append(data_dir +'gtFine_trainvaltest/'+ mask)
  return images, masks
...
def read_images_from_disk(input_queue,
  ...
  if input_size is not None:
    ...
    img, label = divide_into_parts(img, label, region_size, overlap=0)
