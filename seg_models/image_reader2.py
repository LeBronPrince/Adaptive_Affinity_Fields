...
def read_labeled_image_list(data_dir, data_list):
  ...
  for line in f:
    try:
      image, mask, ndsm = line.strip("\n").split(' ')
    except ValueError: # Adhoc for test.
      image = mask = line.strip("\n")
    images.append(data_dir + image)
    masks.append(data_dir + mask)
    ndsms.append(data_dir + ndsm)
  return images, masks, ndsms
...
def read_images_from_disk(input_queue,
  ...
  ndsm_contents = tf.read_file(input_queue[2])

  img = tf.image.decode_png(img_contents, channels=3)
  ndsm = tf.image.decode_png(ndsm_contents, channels=1)
  img = tf.concat([img, ndsm], axis=-1)
...
class ImageReader(object):
  ...
  def __init__(self, data_dir, data_list, input_size,
    ...
    self.image_list, self.label_list, self.ndsm_list = read_labeled_image_list(
        self.data_dir, self.data_list)
    self.images = tf.convert_to_tensor(self.image_list, dtype=tf.string)
    self.labels = tf.convert_to_tensor(self.label_list, dtype=tf.string)
    self.ndsms = tf.convert_to_tensor(self.ndsm_list, dtype=tf.string)
    self.queue = tf.train.slice_input_producer(
        [self.images, self.labels, self.ndsms],
