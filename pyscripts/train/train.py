...
def get_arguments():
  ...
  parser.add_argument('--data-dir', type=str, default='/home/wangyang/Desktop/dataset/VOCdevkit/',
 	             help='/path/to/dataset/.')#/home/wangyang/Desktop/dataset/VOCdevkit/
  parser.add_argument('--data-list', type=str, default='/home/wangyang/Desktop/Semantic Segmentation Code/Adaptive_Affinity_Fields/dataset/voc12/train.txt',
 ...
 # Misc paramters
  parser.add_argument('--restore-from', type=str, default='/home/wangyang/Desktop/Semantic Segmentation Code/Adaptive_Affinity_Fields/resnet_v1_50.ckpt',#
  ...
  parser.add_argument('--snapshot-dir', type=str, default='/home/wangyang/Desktop/Semantic Segmentation Code/Adaptive_Affinity_Fields/snapshot_remote',
...
def main():
  ...
  with tf.device('/cpu:0'):
    with tf.name_scope('create_inputs'):
      ...
      #image_batch = tf.concat([image_batch[0,:,:,:,:],image_batch[1,:,:,:,:],image_batch[2,:,:,:,:],image_batch[3,:,:,:,:]],0)
      #label_batch = tf.concat([label_batch[0,:,:,:,:],label_batch[1,:,:,:,:],label_batch[2,:,:,:,:],label_batch[3,:,:,:,:]],0)
  ...
  restore_var = [
    v for v in tf.global_variables()
      if 'block5' not in v.name and 'struct_multi' not in v.name]
  ...
  for i,output in enumerate(outputs):
    ...
    loss1 = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=output_gather, labels=labels_gather)
    loss = tf.reduce_mean(loss1)
    loss = tf.cond(tf.equal(tf.is_nan(loss),False),lambda:tf.identity(loss),lambda:tf.identity(0.))
  ...
  fc_trainable = [v for v in all_trainable if 'block5' in v.name or 'struct_multi' in v.name] # lr*10
  base_trainable = [v for v in all_trainable if 'block5' not in v.name and 'struct_multi' not in v.name] # lr*1
 ...
 # tf.reset_default_graph()
  ...
  with tf.device('/cpu:0'):
    ...
    #utils.general.decode_labels,[labels_vis, args.num_classes],
    #in_summary = utils.general.decode_labels(image_batch, IMG_MEAN)
    #gt_summary = utils.general.decode_labels(labels_vis, args.num_classes)
    #out_summary = utils.general.decode_labels(output_vis, args.num_classes)
  ...
  for step in pbar:
    ...
    for it in range(args.iter_size):
      ...
      if it == args.iter_size-1 and step % args.update_tb_every == 0:
        ...
        sess_outs = [reduced_loss, train_op,output_vis,labels_vis]
        loss_value, _ ,outputs_out,label_out= sess.run(sess_outs, feed_dict=feed_dict)
        print(np.array(outputs_out).shape)
        print(np.array(label_out).shape)
